...
class ProductController extends Controller
{
    ...
    public function indexDateSort($sort){
        ...
        if ($sort == 'newest') {            
            $products = Product::join('prices', 'products.id', '=', 'prices.priceable_id')->orderBy('created_at', 'ASC')->paginate(12);
        }
        if ($sort == 'oldest') {
            $products = Product::join('prices', 'products.id', '=', 'prices.priceable_id')->orderBy('created_at', 'DESC')->paginate(12);       
        }
        return view('public.products', compact('products'));
        }
    }
